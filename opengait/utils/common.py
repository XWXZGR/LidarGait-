...
def get_valid_args(obj, input_args, free_keys=[]):
    ...
    unexpect_keys = list()
    expected_args = {}
    #遍历输入参数input_args，将与期望参数列表匹配的参数添加到expected_args中
...
def get_attr_from(sources, name):
    try:
        #尝试从sources[0]中获取指定名称name的属性或方法
        return getattr(sources[0], name)
    except:
        #如果失败，则递归地尝试从剩余的源中获取，直到找到为止。
...
def config_loader(path):
    ...
    with open("E:/SC/OpenGait-master/configs/default.yaml", 'r') as stream:
...
#将模型模块 module 转换为分布式数据并行模块（DistributedDataParallel，DDP）的形式。DDP 是 PyTorch 中用于分布式训练的一种并行化策略。
