...
class VerticalPoolingPyramid():
    """
        Vertical Pyramid Pooling
    """

    def __init__(self, bin_num=None):
        if bin_num is None:
            bin_num = [16, 8, 4, 2, 1]  # 默认的分块数量
        self.bin_num = bin_num
    ...
    def __call__(self, x):
        """
            ...
            ret: [n, c, p]
        """
        n, c = x.size()[:2]  # 提取批量大小和通道数
        features = []
        for b in self.bin_num:
            z = x.view(n, c, -1, b)  # 沿着垂直方向分块
            z = z.mean(2) + z.max(2)[0]  # 对每个分块进行池化操作
            features.append(z)
        return torch.cat(features, -1)  # 拼接所有分块的池化结果
