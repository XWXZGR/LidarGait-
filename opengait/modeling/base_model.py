...
#from . import backbones会导入该文件夹中的__init__.py文件。
# 在__init__.py文件中，可能会使用类似循环导入的方式将文件夹中的各个模块导入到backbones模块的命名空间中，这样就可以通过backbones模块来访问该文件夹中的其他模块和类。
...
#__all__是一个Python模块级别的变量，用于定义模块导出的公共接口。
# 在这种情况下，__all__中包含了字符串'BaseModel'，这意味着只有BaseModel类会被导出到模块的命名空间中，其他所有的类和函数都不会被导出。
__all__ = ['BaseModel']

#MetaModel 定义了模型需要实现的抽象方法,BaseModel 则是这些抽象方法的具体实现。
class MetaModel(metaclass=ABCMeta):
    ...
    def get_loader(self, data_cfg):
        ...
        #raise NotImplementedError 是 Python 中的一种异常抛出方式，意思是“尚未实现”。
        # 在这个上下文中，它通常用于抽象方法的定义，表示子类必须实现该方法，否则在调用时会引发此异常。
...
class BaseModel(MetaModel, nn.Module):
    ...
    def get_backbone(self, backbone_cfg):
        ...
        if is_dict(backbone_cfg):
            #通过get_attr_from方法从backbones模块中获取相应类型的主干网络
            Backbone = get_attr_from([backbones], backbone_cfg['type'])
            #使用valid_args参数来获取合法的参数，然后实例化主干网络并返回。
            valid_args = get_valid_args(Backbone, backbone_cfg, ['type'])
            return Backbone(**valid_args)
        #如果backbone_cfg是列表类型，则表示存在多个主干网络。在这种情况下，它会递归地调用
    ...
    def inputs_pretreament(self, inputs):
        ...
        #根据训练模式选择是否使用训练数据的转换操作进行数据增强
        seq_trfs = self.trainer_trfs if self.training else self.evaluator_trfs
        #检查输入的数据序列数量与转换操作数量是否相同
        if len(seqs_batch) != len(seq_trfs):
            raise ValueError(
                "The number of types of input data and transform should be same. But got {} and {}".format(len(seqs_batch), len(seq_trfs)))

        #定义一个转换操作列表，包括调整图片尺寸为224x224
        resize_transform = transforms.Resize((224, 224))

        #将 requires_grad 设置为 True（如果处于训练模式）或 False（如果处于评估模式），以便在需要时计算梯度。
        requires_grad = bool(self.training)
        #对每个序列数据应用相应的转换操作，并将转换后的数据存储在列表 seqs 中
        #使用 zip(seq_trfs, seqs_batch) 来同时遍历 seq_trfs 和 seqs_batch 这两个列表中的元素。seq_trfs 包含了要应用于每个序列的转换函数，而 seqs_batch 包含了多个输入序列。
        #对于每个序列 seq，以及对应的转换函数 trf，列表推导式 trf(fra) for fra in seq 将转换函数应用于序列中的每一帧 fra。这将得到一个由转换后的帧组成的列表
    ...
    def train_step(self, loss_sum) -> bool:
        ...
        if self.engine_cfg['enable_float16']:
            ...
            loss_sum.backward()
            self.optimizer.step() #更新学习率

        self.iteration += 1 #迭代次数加 1
        self.scheduler.step()
        return True
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader: #迭代训练数据加载器中的批次数据
            ipts = model.inputs_pretreament(inputs) #准备输入数据
            with autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts)
                training_feat, visual_summary = retval['training_feat'], retval['visual_summary']
                del retval
            loss_sum, loss_info = model.loss_aggregator(training_feat)  # 计算损失
    ...
    def run_test(model):
        ...
        if rank == 0:
            #获取模型的测试数据加载器 test_loader，并从中获取标签、类型和视图列表
