...
class ResNet9_mul(ResNet):
    def __init__(self, block, channels=[64, 128, 256, 512], in_channel=1, layers=[1, 2, 2, 1], strides=[1, 2, 2, 1],
        ...
        super(ResNet9_mul, self).__init__(block, layers)
    ...
    def forward(self, x):
        ...
        # 从高层到低层进行上采样和融合
        p4_upsampled = F.interpolate(p4, scale_factor=1, mode='nearest')
        ...
        p2 = p2 + p3_upsampled
        print('p2:', p2.shape)
        p2_upsampled = F.interpolate(p2, scale_factor=2, mode='nearest')
        ...
        print('p2_downsampled:', p2.shape)
        # p1 = F.interpolate(p1, scale_factor=0.25, mode='nearest')
        # print('p1_downsampled:', p1.shape)


        # 融合所有尺度的特征
        fused_feature = p4 + p3 + p2
