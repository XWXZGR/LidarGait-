...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    # 定义 probe 序列和 gallery 序列的字典，其中包含了不同数据集下各种类型的序列
    ...
    # 定义 gallery 序列的字典，包含了不同数据集下的图库序列
    ...
    msg_mgr = get_msg_mgr()
    acc = {}
    view_list = sorted(np.unique(view)) # 获取所有不同的视角
    num_rank = 1
    if dataset == 'CASIA-E':
        view_list.remove("270")
    if dataset == 'SUSTech1K':
        num_rank = 5 
    view_num = len(view_list) # 获取视角数量
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        # 遍历每个 probe 视角
        for (v1, probe_view) in enumerate(view_list):
            # 创建 probe 序列的掩码，用于过滤出符合当前类型和视角的特征
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
                view, probe_view)
            # 对于 SUSTech1K 数据集，使用更复杂的掩码
            ...
            for (v2, gallery_view) in enumerate(view_list):
                # 创建 gallery 序列的掩码，用于过滤出符合当前视角的特征
                ...
                # 提取 gallery 特征
                gallery_x = feature[gseq_mask, :]
                # 计算 probe 特征与 gallery 特征之间的距离
                dist = cuda_dist(probe_x, gallery_x, metric)
                # 获取距离最近的 num_rank 个索引
