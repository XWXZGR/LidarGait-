...
#首先检查是否提供了裁剪参数。如果没有提供，则根据输入图像的宽度自动计算裁剪参数，以保留中心部分并去除左右两侧。然后将像素值除以255.0进行归一化。
...
class BaseSilCuttingTransform():
    ...
    def __call__(self, x):
        ...
        if self.cutting is not None:
            ...
            #根据输入图像的宽度自动计算裁剪参数，以保留中心部分并去除左右两侧
            cutting = int(x.shape[-1] // 64) * 10
        if cutting != 0: 
            x = x[..., cutting:-cutting]
        #将像素值除以指定的除数（默认为 255.0），以使图像数据的像素值落在 [0, 1] 的范围内。
        return x / self.divsor

#对RGB图像进行归一化，使用指定的均值和标准差
...
def show_image(image,title):
    if image.shape[0] == 3 and image.shape[1] == 64 and image.shape[2] == 64:  # 检查是否为三通道图片
...
# 随机左右翻转图像
class RandomHorizontalFlip_lidar(object):
    def __init__(self, prob=0.5):
        self.prob = prob
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            for i in range(len(flipped_seq)):
                # 翻转图像
                flipped_seq[i] = np.flipud(flipped_seq[i])

                # 可视化处理前后的图像
                show_image(seq[i], f"Original Image {i+1}")
                show_image(flipped_seq[i], f"Flipped Image {i+1}")
            return flipped_seq
...
#随机擦除图像的一部分
class RandomErasing_lidar(object):
    def __init__(self, prob=0.5, sl=0.05, sh=0.2, r1=0.3, per_frame=False):
        self.prob = prob
        self.sl = sl
        self.sh = sh
    ...
    def __call__(self, seq):
        for i in range(seq.shape[0]):
            # # 可视化处理前的图像
            # show_image(seq[i], "Original Image")
            ...
            if not self.per_frame:
                if random.uniform(0, 1) < self.prob:
                    ...
                    seq_size = seq.shape
                    area = seq_size[2] * seq_size[3]
                    target_area = random.uniform(self.sl, self.sh) * area
                    aspect_ratio = random.uniform(self.r1, 1 / self.r1)
                    h = int(round(math.sqrt(target_area * aspect_ratio)))
                    w = int(round(math.sqrt(target_area / aspect_ratio)))
                    if w < seq_size[3] and h < seq_size[2]:
                        x1 = random.randint(0, seq_size[2] - h)
                        y1 = random.randint(0, seq_size[3] - w)
                        for c in range(seq_size[1]):
                            seq[i, c, x1:x1 + h, y1:y1 + w] = 0.

                    # # 可视化处理后的图像
                    # show_image(seq[i], "Erased Image")
            else:
                self.per_frame = False
                frame_num = seq.shape[0]
                ret = [self.__call__(seq[k]) for k in range(frame_num)]
                self.per_frame = True
                seq = np.concatenate(ret, 0)
        return seq
...
#原始随机旋转图像
class RandomRotate_lidar(object):
    def __init__(self, prob=0.5, degree=10):
        self.prob = prob
        self.degree = degree
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            num_frames, num_channels, dh, dw = seq.shape
            rotated_seq = np.zeros_like(seq)
            center = (dw // 2, dh // 2)  # 设置旋转中心为图像中心
            for i in range(num_frames):
                ...
                for c in range(num_channels):
                    # 进行仿射变换
                    rotated_frame = cv2.warpAffine(seq[i, c], M, (dw, dh))
                    # 将旋转后的图像存储到旋转序列中
                    rotated_seq[i, c] = rotated_frame

                # # 可视化旋转前后的图像
                # show_image(seq[i], 'Original Image')
                # show_image(rotated_seq[i], f"Rotated Image {i+1}")
...
class RandomPerspective(object):
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            return seq
##透视变换
class RandomPerspective_lidar(object):
    def __init__(self, prob=0.5):
        self.prob = prob
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            num_frames, num_channels, dh, dw = seq.shape
            transformed_seq = np.zeros_like(seq)
            for i in range(num_frames):
                # 随机生成透视变换的参数
                cutting = int(dw // 44) * 10
                x_left = list(range(0, cutting))
                x_right = list(range(dw - cutting, dw))
                TL = (random.choice(x_left), 0)
                TR = (random.choice(x_right), 0)
                BL = (random.choice(x_left), dh)
                BR = (random.choice(x_right), dh)
                src_points = np.float32([TL, TR, BR, BL])
                canvas_points = np.float32([[0, 0], [dw, 0], [dw, dh], [0, dh]])
                perspective_matrix = cv2.getPerspectiveTransform(src_points, canvas_points)
                # 对当前帧的每个通道进行透视变换
                for c in range(num_channels):
                    transformed_frame = cv2.warpPerspective(seq[i, c], perspective_matrix, (dw, dh))
                    transformed_seq[i, c] = transformed_frame

                # # 可视化处理前后的图像
                # show_image(seq[i], f'Original Image {i+1}')
                # show_image(transformed_seq[i], f'Transformed Image {i+1}')
...
# class RandomAffine_lidar(object):
#     def __init__(self, prob=0.5, max_shift=10):
#         self.prob = prob
...
#                 center_x = dw // 2
#                 center_y = dh // 2
#                 # 随机生成水平平移量，限制在[-max_shift, max_shift]范围内
#                 tx = random.randint(-self.max_shift, self.max_shift)
#                 # 计算平移后的中心位置
#                 new_center_x = center_x + tx
#                 # 计算平移量
#                 shift = new_center_x - center_x
#                 # 构建平移矩阵
#                 M = np.float32([[1, 0, shift], [0, 1, 0]])
#                 # 对当前帧进行平移操作
#                 transformed_frame = cv2.warpAffine(seq[i], M, (seq[i].shape[1], seq[i].shape[0]))
#
#                 # 可视化处理前后的图像
#                 show_image(seq[i], f'Original Image {i+1}')
#                 show_image(transformed_frame, f'Transformed Image {i+1}')
#
#                 transformed_seq[i] = transformed_frame
#
#             return transformed_seq

class RandomAffine_lidar(object):
    def __init__(self, prob=0.5, max_shift=2):
        self.prob = prob
        self.max_shift = max_shift
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            for i in range(num_frames):
                ...
                center_x = dw // 2
                center_y = dh // 2
                # 随机生成水平和垂直平移量，限制在[-max_shift, max_shift]范围内
                tx = random.randint(-self.max_shift, self.max_shift)
                ty = random.randint(-self.max_shift, self.max_shift)
                # 构建平移矩阵
                M = np.float32([[1, 0, tx], [0, 1, ty]])
                # 对当前帧进行平移操作，边界填充为黑色
                transformed_frame = cv2.warpAffine(seq[i], M, (seq[i].shape[1], seq[i].shape[0]), borderMode=cv2.BORDER_CONSTANT, borderValue=0)
...
# ******************************************
#Compose 函数的作用是将多个数据转换方法组合成一个整体的数据转换管道。
def Compose(trf_cfg):
...
# **************** For pose-based methods 姿态数据转换操作 ****************
